...
ไม่มี role · `event.author_id` มีไว้**แสดงว่าใครลงเท่านั้น ไม่ใช่สิทธิ์** ทั้งสองคนยังแก้และลบของกันได้หมด
ใส่ตอน INSERT อย่างเดียว แก้ทีหลังไม่เปลี่ยนเจ้าของ · ของที่ลงไว้ก่อนมีคอลัมน์นี้เป็น NULL
แล้วการ์ดจะไม่โชว์ avatar (ย้อนไปดูไม่ได้ว่าใครลง เดาแล้วใส่ผิดแย่กว่าปล่อยว่าง) —
อยากยกให้เป็นของใครทั้งหมดใช้ `UPDATE event SET author_id = <id> WHERE author_id IS NULL`
