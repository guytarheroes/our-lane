...
## ยังไม่ได้ทำ: สลับภาษา ไทย/อังกฤษ

ยังไม่ได้เริ่ม — บันทึกจุดที่จะพลาดง่ายไว้ก่อน

**เก็บภาษาแบบเดียวกับธีม** cookie + `Base.astro` render ตั้งแต่ฝั่ง server
ห้ามใช้ JS อ่าน localStorage เพราะจะเกิดจอกระพริบ และห้ามใช้ไลบรารี i18n
(กฎห้ามเพิ่ม dependency) — `lib/i18n.js` ที่เป็น object สองภาษาก็พอสำหรับเว็บขนาดนี้
ต้องมี `validLang()` กรอง cookie แบบเดียวกับ `validTheme()` ไม่งั้นค่าปลอมหลุดลง `lang`

**`<html lang>` ต้องเปลี่ยนตามด้วย** ตอนนี้ hardcode `lang="th"` ไว้ใน `Base.astro`
ค่านี้ไม่ใช่แค่ metadata — เบราว์เซอร์ใช้มันตัดคำภาษาไทย ถ้าลืมเปลี่ยนเป็น `en`
ข้อความอังกฤษจะถูกตัดบรรทัดด้วยกฎภาษาไทย

**วันที่คือส่วนที่ยากที่สุด ไม่ใช่แค่แปลคำ**
* `th-TH` ให้ปี **พุทธศักราช** ส่วน `en-*` ให้คริสต์ศักราช — ตัวเลขปีบนหน้าเดียวกันจะต่างกัน 543
* `lib/since.js` มี `sinceThai()` ที่ประกอบสตริงไทยเอง (`"3 ปี 6 เดือน 14 วัน"`)
  ต้องแยกส่วนคำนวณ (`since()` ใช้ร่วมกันได้) ออกจากส่วนประกอบข้อความ
* `lib/calendar.js` มี `todayISO()` ที่**ห้าม**เปลี่ยนไปใช้ locale ใด ๆ — มันคือคีย์ของ DB
  ต้องเป็น `YYYY-MM-DD` ตามเวลาท้องถิ่นเสมอ
* ชื่อวันย่อในปฏิทิน (`อา จ อ...`) hardcode เป็น array อยู่ใน `calendar.astro`
...
**`COUPLE_NAME` ไม่ต้องแปล** เป็นค่าที่เจ้าของตั้งเองใน `/settings`
