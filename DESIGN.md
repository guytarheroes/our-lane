...
**ฟอนต์:** **ไทยใช้ Bai Jamjuree · อังกฤษใช้ Pixellari (ฟอนต์พิกเซล) ทั้งหน้า**
สลับด้วย `html[lang='en']` ที่ `Base.astro` render มาจาก server ไม่ต้องใช้ JS

Pixellari ไม่มีตัวอักษรไทยเลยสักตัว — ในโหมดไทยจึงใช้ได้ที่เดียวคือ wordmark
`our memory lane` (คลาส `.pixel`) ที่เป็นละตินล้วนโดยตั้งใจ ถ้าเอาไปใส่ที่อื่น
ข้อความไทยจะตกกลับไป Bai Jamjuree แล้วบรรทัดเดียวกันจะมีตัวพิกเซลปนตัวเรียบ

ข้อควรระวังของฟอนต์พิกเซล — ผิดข้อไหนก็เบลอทันที:
* **ขนาดต้องเป็นเท่าตัวของ 16px** (em = 1024 หน่วย = 64 หน่วยต่อพิกเซล) `.pixel` ตรึงไว้ที่ 16px แล้ว
  ส่วนโหมดอังกฤษใช้ขนาดเดิมของแต่ละหน้าซึ่งไม่ตรงกริด แต่ปิด antialias ไว้เลยยังคมพอ
* **ปิด antialias เสมอ** (`-webkit-font-smoothing: none`) ไม่งั้นขอบพิกเซลถูกเกลี่ยจนหายคม
* **`font-synthesis: none`** ฟอนต์มีน้ำหนักเดียว ปล่อยไว้เบราว์เซอร์จะปลอมตัวหนาให้แล้วเละ
* **`letter-spacing` เป็น px จำนวนเต็ม** ไม่ใช่ `em` ไม่งั้นตัวอักษรตกครึ่งพิกเซล
