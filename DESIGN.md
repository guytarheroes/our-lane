...
**`--accent` ตั้งเองได้ที่ `/settings`** เก็บใน DB (แชร์กันทั้งสองคน) แล้ว `Base.astro`
ฉีดทับด้วย `html:root` ที่ specificity สูงกว่า `:root` — ไม่ต้องพึ่งลำดับที่ Astro แทรก CSS
`lib/color.js` วัด contrast แล้วปฏิเสธสีที่อ่านไม่ออกพร้อมบอกเหตุผล **อย่าถอดด่านนี้ออก**
เกณฑ์คนละอันเพราะบทบาทคนละอย่าง: โหมดสว่าง accent เป็นตัวอักษรด้วย (4.5) ส่วนโหมดมืด
เป็นแค่พื้นปุ่ม/จุด (3.0 บนพื้น + 4.5 เทียบตัวหนังสือบนปุ่ม) และ **โหมดมืดห้ามทับ
`--accent-ink`** ไม่งั้นลิงก์จะกลายเป็นสีเข้มบนพื้นมืด
